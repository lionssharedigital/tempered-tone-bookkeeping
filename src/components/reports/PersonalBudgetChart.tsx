...
// Same categorical slot used for "Expense" on the P&L chart, for visual
// consistency across the two reports.
...
export default function PersonalBudgetChart({ monthly }: { monthly: number[] }) {
  const [hovered, setHovered] = useState<HoveredBar | null>(null);

  const maxCents = useMemo(() => niceCeiling(Math.max(0, ...monthly) || 100), [monthly]);

  const gridlineCount = 4;
  const gridlines = Array.from({ length: gridlineCount + 1 }, (_, i) => (maxCents / gridlineCount) * i);

  const groupWidth = PLOT_WIDTH / 12;
  const barWidth = Math.min(BAR_MAX_WIDTH, groupWidth * 0.6);
    ...
    <div className="surface-card relative mb-6 p-4">
      ...
      <div className="overflow-x-auto">
        <svg
          ...
          style={{ minWidth: 600 }}
          role="img"
          aria-label="Monthly personal spending chart"
        >
          <g transform={`translate(${MARGIN.left},${MARGIN.top})`}>
            {gridlines.map((value, i) => {
              ...
              return (
                <g key={i}>
                  <line x1={0} x2={PLOT_WIDTH} y1={y} y2={y} stroke="var(--grid-line)" strokeWidth={1} />
                  <text
                    x={-8}
                    y={y}
                    textAnchor="end"
            ...
            {MONTH_LABELS.map((month, i) => {
              ...
              const barX = i * groupWidth + (groupWidth - barWidth) / 2;
              ...
              return (
                <g key={month}>
                  ...
                  <rect
                    ...
                    y={barY}
                    width={barWidth}
                    height={Math.max(0, PLOT_HEIGHT - barY)}
                    rx={4}
                    fill={SPEND_COLOR}
      ...
      {hovered && (
        ...
        >
          ...
          <div className="text-text-muted">{hovered.month}</div>
        </div>
      )}
    </div>
  );
